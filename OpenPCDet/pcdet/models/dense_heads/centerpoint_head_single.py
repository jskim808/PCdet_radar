...
class CenterHead(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        if not self.training or self.predict_boxes_when_training:
            batch_cls_preds, batch_box_preds = self.generate_predicted_boxes_v2(
        ...
        for idx in range(1):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    ##make 2d target (x,y,w,l,theta)
                    anno_box[new_idx] = torch.cat([
                        center - torch.tensor([x, y], device=device),
                        box_dim[0:2],  rot.unsqueeze(0),
                    ])
    ...
    def generate_predicted_boxes_v2(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        """
        Args:
            batch_size:
            cls_preds: (N, H, W, C1)
        ...
        batch_dim = torch.exp(box_preds[..., 2:4])
        batch_dimz = 2.0
        ...
        batch_box_preds = torch.cat([xs, ys, batch_hei, batch_dim, batch_dimz, rot], dim=2)
    ...
    def get_box_reg_layer_loss(self):
        ...
        ind = inds
        num = masks.float().sum()
        pred = self.forward_ret_dict['box_preds'] # N x (HxW) x 5 
