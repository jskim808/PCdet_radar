...
def idxing_point_box_in_boxes3d(points, boxes3d):
    boxes3d, is_numpy = common_utils.check_numpy_to_torch(boxes3d)
    points, is_numpy = common_utils.check_numpy_to_torch(points)
    point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(points[:, 0:3], boxes3d)
    
    outpoints = points[point_masks.sum(dim=0) == 0]
    inpoints = points[point_masks.sum(dim=0) != 0]
    inboxes = boxes3d[point_masks.sum(dim=1) != 0]
    in_points = point_masks.sum(dim=1).numpy()
    box_idx = point_masks.sum(dim=1) != 0
    
    return in_points, box_idx
