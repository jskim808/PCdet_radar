...
class ResidualBEVCoder(object):
    def __init__(self, code_size=5, encode_angle_by_sincos=False, **kwargs):
        super().__init__()
        self.code_size = code_size
        self.box_size = 7
        self.encode_angle_by_sincos = encode_angle_by_sincos
    ...
    def encode_torch(self, boxes, anchors):
        """
        Args:
            boxes: (N, 7 + C) [x, y, dx, dy, heading, ...]
            anchors: (N, 7 + C) [x, y, dx, dy, heading or *[cos, sin], ...]
        ...
        cts = [g - a for g, a in zip(cgs, cas)]
        return torch.cat([xt, yt, dxt, dyt, *rts, *cts], dim=-1)

    def decode_torch(self, box_encodings, anchors):
        """
        Args:
            box_encodings: (B, N, 5 + C) or (N, 5 + C) [x, y, dx, dy, heading or *[cos, sin], ...]
            anchors: (B, N, 7 + C) or (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
        ...
        xa, ya, za, dxa, dya, dza, ra, *cas = torch.split(anchors, 1, dim=-1)
        if not self.encode_angle_by_sincos:
            xt, yt, dxt, dyt, rt, *cts = torch.split(box_encodings, 1, dim=-1)
        else:
            xt, yt, dxt, dyt, cost, sint, *cts = torch.split(box_encodings, 1, dim=-1)
        ...
        cgs = [t + a for t, a in zip(cts, cas)]
        ret = torch.cat([xg, yg, dxg, dyg, rg, *cgs], dim=-1)
        z_fixed = torch.full([*ret.shape[:-1], 1],-1.0,dtype=ret.dtype,device=ret.device)
        h_fixed = torch.full([*ret.shape[:-1], 1],2.0,dtype=ret.dtype,device=ret.device)
